	...
	if (Ingame) {
			...
			case 0:
				...
				case 1: case 2:
					...
					case 0:
						...
						ReflectorPosition(ReflectorHead, (GameStatus == 1), GetAsyncKeyState(Reflector1Left), GetAsyncKeyState(Reflector1Right), GetAsyncKeyState(Reflector1Up), GetAsyncKeyState(Reflector1Down));
						break;
					case 1:
						if (Reactor.meltdown == false and Orbcount < 0) {
							Orbcount = 0;
		...
		case WM_PAINT:
			...
			if (DisplayGame) {
				...
				else if (GameStatus == 0) Menu_UI(SelectedButton, L"o ESC\\", L"", L"Resume", L"Options", L"Quit To Main Menu");
			}
		...
		case WM_KEYDOWN:
			if (wParam == VK_ESCAPE) {
				...
				if (Menu_Type == 2) Menu_Type = 0;
				if (Menu_Type == 3) Menu_Type = 0;
				if (Menu_Type / 10 == 3) Menu_Type = 3;
			}
			break;
		case WM_MOUSEMOVE:
		...
		case WM_PAINT:
			...
			if (Menu_Type == 0) Menu_UI(SelectedButton, L"Power Ing\\", L"Start!", L"Module", L"Options", L"Quit");
			else if (Menu_Type == 1) Menu_UI(SelectedButton, L"Game Mode", L"Co-Op", L"HS-Competive", L"Competive", L"");
			else if (Menu_Type == 2) Menu_UI(SelectedButton, L"Module", L"Color", L"", L"Module", L"");
			else if (Menu_Type == 3) Menu_UI(SelectedButton, L"Options", L"GamePlay", L"Display", L"Sound", L"");
			else if (Menu_Type == 31) Menu_UI(SelectedButton, L"GamePlay", L"Control", L"Reset Speed Setting?", L"Score", L"");
			else if (Menu_Type == 32) Menu_UI(SelectedButton, L"Display", L"Resolution", L"Trail Quality", L"", L"");
			else if (Menu_Type == 33) Menu_UI(SelectedButton, L"Sound", L"Master Volum", L"Volum", L"Alert Setting", L"");
