	...
	if (Ingame) {
		...
		case WM_KEYDOWN:
			...
			if (wParam == VK_ESCAPE) {
				...
				Player[gClientID].Control[0] = true;
			}
			break;
		case WM_MOUSEMOVE:
			switch (GameStatus) {
				...
				case 1: case 2:
					...
					case 0:
						...
						*/
						...
						Player[gClientID].Control[1] = GetAsyncKeyState(Reflector1Left);
						Player[gClientID].Control[2] = GetAsyncKeyState(Reflector1Right);
						Player[gClientID].Control[3] = GetAsyncKeyState(Reflector1Up);
						Player[gClientID].Control[4] = GetAsyncKeyState(Reflector1Down);

						client.SendPacket(PACKET_TYPE_KEY_INPUT, Player[gClientID]);
						Player[gClientID].Control[0] = false;
		...
		case WM_PAINT:
			...
			if (DisplayGame) {
				...
				ReactorImg.Draw(memdc, int(Pibot_x - Controllroom_half_x), int(Pibot_y - Controllroom_half_y), Controllroom_window_x, Controllroom_window_y, 0, 0, Controllroom_size_x, Controllroom_size_y);
