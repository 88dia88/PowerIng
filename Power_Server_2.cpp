...
DWORD WINAPI LobbyThread(LPVOID lpParam)
{
	WaitForMultipleObjects(MAX_NUM_CLIENTS, hClientKeyInputEvent, TRUE, INFINITE);
	...
	while (gameState == GAME_STATE_LOBBY) {
		...
		if (allReady) {
			...
			while (gameState == GAME_STATE_READY) {
				if (1) { // 게임 시작 버튼? 누르면
					for (int i = 0; i < MAX_NUM_CLIENTS; ++i) {
						LobbyDataPacket SendLobbyData;
						SendLobbyData.playerCount = 3;
						SendLobbyData.clientID = i;
						SendLobbyData.players[i].SetPlayerData(players[i]);
						send(gClientSock[i], reinterpret_cast<char*>(&SendLobbyData), sizeof(LobbyDataPacket), 0);
					}					
					allReady = false;
...
DWORD WINAPI ClientThread(LPVOID arg)
{
	...
	printf("[서버] 클라이언트 종료: ID=%d, IP 주소=%s, 포트 번호=%d\n",
